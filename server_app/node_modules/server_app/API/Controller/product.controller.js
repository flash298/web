...
// Hàm này dùng để hiện những sản phẩm random theo giới tính
module.exports.random = async (req, res) => {
    try {
        ...
        if (!categories.length) {
            return res.json({ msg: "Không tìm thấy danh mục nào với giới tính này." });
        }

        const categoryIds = categories.map((category) => category._id);

        const products = await Products.find({ id_category: { $in: categoryIds } }).limit(4);

        if (!products.length) {
            return res.json({ msg: "Không tìm thấy sản phẩm nào thuộc danh mục này." });
        }

        res.json(products);
    } catch (err) {
        console.error(err);
        res.json({ msg: "Đã xảy ra lỗi." });
    }
};